...
// ── Scalar types ───────────────────────────────────────────────────
internal data class WithShort(val x: Short)
internal data class WithByte(val x: Byte)
internal data class WithChar(val c: Char)
internal data class WithBigDecimal(val amount: BigDecimal)
internal data class WithByteArray(val blob: ByteArray)
internal data class WithArrayOfString(val tags: Array<String>)

// ── Transparent value class wrappers ───────────────────────────────
internal data class WithDuration(val timeout: Duration)
internal data class WithUInt(val id: UInt)

// ── Generic data class ─────────────────────────────────────────────
internal data class Box<T>(val value: T, val label: String)
internal data class ContainerWithGeneric(val box: Box<String>, val count: Int)

// ── Recursive type ─────────────────────────────────────────────────
internal data class Tree(val value: Int, val left: Tree?, val right: Tree?)
...
internal data class Triangle(val base: Double, val height: Double) : Polygon
internal data class Rectangle(val width: Double, val height: Double) : Polygon
...
class DataFrameGapTest : SparkTestBase() {
    ...
    @Test fun `BigDecimal round-trip`() {
    ...
    @Test fun `Array of String round-trip`() {
        val result = listOf(WithArrayOfString(arrayOf("a", "b", "c"))).toDataFrame(spark).toKotlinList<WithArrayOfString>()
        assertArrayEquals(arrayOf("a", "b", "c"), result[0].tags)
    }

    // ── Value class wrappers — expected to work transparently ──────

    @Test fun `kotlin Duration round-trip`() {
        val result = listOf(WithDuration(5.seconds), WithDuration(10.seconds)).toDataFrame(spark).toKotlinList<WithDuration>()
        assertEquals(5.seconds, result[0].timeout)
        assertEquals(10.seconds, result[1].timeout)
    }

    @Test fun `UInt round-trip`() {
    ...
    @Test fun `recursive type schema inference throws clearly`() {
        val ex = assertThrows(IllegalArgumentException::class.java) {
            getSparkSchema(typeOf<Tree>())
        }
        assertTrue(ex.message!!.contains("recursive", ignoreCase = true),
            "Expected message to mention 'recursive', was: ${ex.message}")
    }

    // ── Multi-level sealed — expected to miss Polygon subclass fields

    @Test fun `multi-level sealed round-trip`() {
