...
class BackendBenchmarkTest : SparkTestBase() {
    ...
    private fun measure(block: () -> Any?): List<Long> =
        (1..MEASURE_ROUNDS).map {
            measureNanoTime { blackhole(retryOnUnavailable { block() }) }
        }
    ...
    private fun appendCsv(
        ...
        if (!file.exists()) {
            file.writeText(
                "timestamp,backend,workload,n_rows,warmup_rounds," +
        ...
        file.appendText(
            "$ts,$backend,$workload,$n,$WARMUP_ROUNDS,$MEASURE_ROUNDS," +
                "${
                    String.format(
                        Locale.US,
                        "%.1f",
