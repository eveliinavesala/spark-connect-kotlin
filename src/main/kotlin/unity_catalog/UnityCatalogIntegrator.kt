...
object UnityCatalogIntegrator {
    ...
    inline fun <reified T : Any> registerTable(
        ...
        val sql = generateCreateSQL<T>(tableName, format, location)
    ...
    fun buildCreateStatement(
        ...
        val columns = schema.fields().joinToString(",\n  ") { field ->
            val type = field.dataType().sql()
            val nullable = if (field.nullable()) "" else " NOT NULL"
            val comment = if (field.metadata().contains("comment")) " COMMENT '${field.metadata().getString("comment")}'" else ""
