...
class ScdFailoverTest : SparkTestBase() {
    ...
    fun `SCD field removed - nullable migration (V3) succeeds with reflection fallback`() {
        ...
        val v1ShapeDf = v2Df.drop("score")
        val migratedDf =
            v1ShapeDf.withColumn(
                "score",
                org.apache.spark.sql.functions
                    .lit(null)
                    .cast(org.apache.spark.sql.types.DataTypes.IntegerType),
            )
        ...
        assertTrue(
            result is RouterResult.SerializationSuccess<*>,
            "Expected SerializationSuccess but got ${result::class.simpleName}",
        )
    ...
    fun `SCD field added to data - decode succeeds but new field is silently ignored`() {
        ...
        when (val result = BackendRouter.decode(v2Df, serializer<CustomerV1>())) {
            ...
            is RouterResult.SerializationSuccess -> {
                ...
                assertTrue(
                    diffs.any { it.kind == DriftKind.FIELD_ADDED },
                    "Pre-flight must surface the new 'score' column even when decode succeeds",
                )
            }

            is RouterResult.BothFailed -> {
                fail("Both backends should not fail when data has extra columns")
            }
        }
    }
    ...
    fun `SCD type changed - pre-flight flags TYPE_CHANGED drift`() {
        ...
        assertTrue(
            diffs.any { it.fieldName == "tier" && it.kind == DriftKind.TYPE_CHANGED },
