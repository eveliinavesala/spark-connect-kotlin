...
data class SchemaDriftReport(
    ...
    fun format(): String =
        buildString {
            ...
            expectedSchema.fields().forEach { f ->
                ...
                appendLine(
                    "║    ${f.name().padEnd(22)} ${
                        f.dataType().simpleString().padEnd(14)
    ...
    companion object {
        ...
        fun compare(
            ...
            for ((name, expType) in expectedMap) {
                when (val actType = actualMap[name]) {
                    null -> {
                        diffs += FieldDiff(name, expType, null, DriftKind.FIELD_REMOVED)
                    }
                    ...
                    expType -> { // match — no drift
                        ...
                        diffs += FieldDiff(name, expType, actType, DriftKind.TYPE_CHANGED)
                    }
                }
            }
        ...
        fun triggerFrom(diffs: List<FieldDiff>): DriftTrigger =
            when {
